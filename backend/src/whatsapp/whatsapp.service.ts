    ...
    private async handleAutoReply(userId: string, jid: string, text: string, name: string) {
        ...
        if (currentState === 'MENU') {
            if (msg === '1' || msg.includes('site') || msg.includes('comprar')) {
                ...
                await this.handleProductSearch(userId, jid, msg, user);
            }
...
🕐 _Atendimento 24h_`;
    ...
    private async handleProductSearch(userId: string, jid: string, query: string, user: any) {
        const storeName = user?.storeName || "Loja";
        ...
        if (query) {
            ...
            const scored = allProducts.map(p => {
                ...
                const searchStr = `${pName} ${pBrand} ${pModel}`;
                ...
                for (const token of tokens) {
                    ...
                    if (pDesc.includes(token)) score += 0.5;
                }
        ...
        if (found.length > 0) {
            const limit = 3;
            const products: any[] = found.slice(0, limit);
            ...
            for (const product of products) {
                ...
                if (product.images && product.images.length > 0) {
                    await this.sendImage(userId, jid, this.resolveImageUrl(product.images[0]));
...
💰 *R$ ${price}*`;
            ...
            await this.sendMessage(userId, jid, "😕 Não encontrei nenhum produto com essas características. Tente buscar apenas pelo *nome* ou *marca* (ex: ASUS, i5, Mouse).");
            await this.sendMainMenu(userId, jid, storeName);
        }
    }
